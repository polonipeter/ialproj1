...
void Stack_Init( Stack *stack ) {
    ...
    if (stack == NULL){Stack_Error(SERR_INIT);}//pokial pride NULL vola sa StackError
    else{stack->topIndex = -1;}//zasobnik je prazdny nastavuje sa hodnota -1
...
int Stack_IsEmpty( const Stack *stack ) {
    ...
    return stack->topIndex == -1;//pokial nie je zasobnik prazdny vracia sa hodnota falsw
                                 //teda 0 inak je podmienka platna teda sa vracia 1
}
...
int Stack_IsFull( const Stack *stack ) {
    ...
    return stack->topIndex == STACK_SIZE-1;//od stack_size musim odratat 1 pretoze sa indexuje od 0
}
...
void Stack_Top( const Stack *stack, char *dataPtr ) {
    ...
    if(Stack_IsEmpty(stack)!=0){Stack_Error(SERR_TOP);}//nekorektne volanie funkcie pretoze je zasobnik prazndy
    else{*dataPtr = stack->array[stack->topIndex];}//z pola ulozenych hodnot vyberiem posledny prvok a ulozim ho do dataPtr
}
...
void Stack_Pop( Stack *stack ) {
    ...
    if(Stack_IsEmpty(stack)==0){stack->topIndex = stack->topIndex-1;}//znizujem vrchol zasobniku aby som odstranil prvok na vrchole
}
...
void Stack_Push( Stack *stack, char data ) {
    ...
    if(Stack_IsFull(stack)!=0){Stack_Error(SERR_PUSH);}//pokus o vlozenie prvku do plneho zasobniku je nekorektny
    else{
    stack->topIndex += 1;
    stack->array[stack->topIndex] = data;}//na novu hodnotu vrcholu ukladam polozku
}
